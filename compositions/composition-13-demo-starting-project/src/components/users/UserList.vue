...
export default {
  ...
  setup(props, context) {
    ...
    watch(enteredSearchTerm, function(nval) {
      setTimeout(() => {
        if (nval === enteredSearchTerm.value) {
          activeSearchTerm.value = nval;
        }
      }, 300);
    });
    ...
    return { sort, sorting, displayedUsers, updateSearch, enteredSearchTerm };
  }
